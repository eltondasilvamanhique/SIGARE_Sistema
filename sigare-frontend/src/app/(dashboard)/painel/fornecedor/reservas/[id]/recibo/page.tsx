...
interface Recibo {
  ...
  recurso: {
    ...
    fornecedor: { nome: string; contacto?: string; endereco?: string; utilizador: { email: string; telefone?: string } };
  };
  alocacoes: Alocacao[];
}
...
export default function ReciboFornecedorPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/reservas/${params.id}/recibo`)
      ...
      .catch(() => router.push('/painel/fornecedor/reservas'))
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-6 print:hidden">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#0f2554]">
          ← Voltar
        </button>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-[#0f2554] text-white text-sm font-bold rounded-lg hover:bg-[#1a3a7a]"
          >
            🖨️ Imprimir
          </button>
        </div>
        ...
        <div className="flex items-start justify-between border-b border-slate-200 pb-6 mb-6">
          ...
          <div className="text-right">
            ...
            <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium ${
              ...
              recibo.estado === 'devolvida'  ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
            }`}>{estadoLabel[recibo.estado] ?? recibo.estado}</span>
          </div>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-[#0f2554]">{recibo.horas}h</p>
            </div>
            <div>
