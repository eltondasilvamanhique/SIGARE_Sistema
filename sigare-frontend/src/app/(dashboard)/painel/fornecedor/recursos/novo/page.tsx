...
export default function NovoRecursoPage() {
  ...
  async function uploadFoto(): Promise<string | null> {
    ...
    const token = localStorage.getItem('sigare_token');

    // Tenta primeiro com JWT do utilizador, fallback para anon key
    const authHeader = token ? `Bearer ${token}` : `Bearer ${supabaseKey}`;

    const res = await fetch(
      `${supabaseUrl}/storage/v1/object/sigare/${path}`,
      {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
      // fallback: tenta com anon key se falhou com JWT
      if (token) {
        ...
        if (!res2.ok) {
          ...
          setErro('Erro ao fazer upload da foto. Verifique as permissões do bucket.');
          return null;
        }
      } else {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    // Preço diário: se não definido, calcula automaticamente (8h * preço/hora)
    ...
    try {
      ...
      const data = (err as { response?: { data?: { mensagem?: string; erros?: { fieldErrors?: Record<string, string[]> } } } })?.response?.data;
      if (data?.erros?.fieldErrors) {
        const campos = Object.entries(data.erros.fieldErrors)
          .map(([campo, msgs]) => `${campo}: ${(msgs as string[]).join(', ')}`)
          .join(' | ');
        setErro(`Campos inválidos — ${campos}`);
      } else {
        setErro(data?.mensagem ?? 'Erro ao cadastrar recurso.');
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#0f2554]">Cadastrar Novo Recurso</h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              required
              placeholder="Ex: Palco coberto 6x8m"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Dimensões, capacidade, características do recurso..."
              rows={3}
          ...
          {/* Coordenadas GPS (só visível se tiver endereço) */}
          {form.endereco && (
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-slate-700">
                Coordenadas GPS
              ...
              {form.latitude && form.longitude && (
                ...
                >
                  🗺️ Verificar coordenadas no Google Maps →
                </a>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              >
                <option value="">
                  {categorias.length === 0 ? 'A carregar...' : 'Seleccionar...'}
                </option>
                {categorias.map((c) => (
          ...
          <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={form.preco_hora ? `${(Number(form.preco_hora) * 8).toFixed(2)} (auto)` : 'Auto (8×hora)'}
                    className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e4db7]"
                  />
                </div>
              </div>
            ...
            {form.preco_hora && (
              <div className="text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-2">
                Preço diário automático: <strong className="text-[#0f2554]">
                  {(Number(form.preco_hora) * 8).toLocaleString('pt-MZ', { minimumFractionDigits: 2 })} MT
                </strong> {form.preco_dia ? '(substituído pelo valor acima)' : '(8 × preço/hora)'}
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-100">
            ...
            >
              {saving ? 'A guardar...' : 'Cadastrar Recurso'}
            </button>
          </div>
        </form>
      </div>
